...
<!--                    <div class="info_line" id="dataset_description_holder"><div id="dataset_description" data-ng-bind-html="description"></div></div>	-->
                ...
                <div id="paste_wrapper" ng-show="displayPaste">
                    ...
                    {   echo('<div class="data_line">');
                            echo('<div class="data_leftSection">');
                                echo('<div ng-class="pasteClass" ng-click="showData(1)">Paste Text</div>');																
                                echo('<div ng-class="fileClass" ng-click="showData(2)">Text File</div>');																	
                                echo('<div ng-class="excelClass" ng-click="showData(4)">Excel File</div>');																		
                            echo('</div>');
                            echo('<div class="data_rightSection">');
                                echo('<div class="dataBlock" ng-show="dataPaste">Paste Text Below</div>');															
                                echo('<div class="dataBlock" ng-show="dataFile">Select a word, rtf, or text file<input type="file" id="dataset_file" class="textButton" custom-on-change="readFile" /></div>');																
                                echo('<div class="dataBlock" ng-show="dataHTML"><input type="text" id="dataset_html" ng-model="Data.HTMLString"></div>');																
                                echo('<div class="dataBlock" ng-show="dataExcel">Select an excel file<input type="file" id="dataset_excel" class="textButton" custom-on-change-excel="readExcel" /></div>');																
                                echo('<div class="data_actionitem" ng-show="dataOA"><input type="radio" id="textOverwrite" name="parseType" value="radio_overwrite" ng-model="dataAction" ><label for="textOverwrite"><span></span>Overwrite Dataset</label></div>');
                                echo('<div class="data_actionitem" ng-show="dataOA"><input type="radio" id="textAdd" name="parseType" value="radio_adddata" ng-model="dataAction"><label for="textAdd"><span></span>Add Text to Data</label></div>');
                            echo('</div>');
                        echo('</div>');

                        echo('<div class="dataSection" ng-show="!dataAvailable">');
                            echo('<div class="info_line"><div id="parse_dataset" ng-click="parseText()">Parse Data</div></div>');
                            echo('<textarea id="dataset_paste" ng-model="Data.string"></textarea>');
                        echo('</div>');

                        echo('<div class="dataSection" ng-show="dataAvailable">');
                            echo('<div class="settings_line">');
                                echo('<div id="parseGo" ng-click="compileText()">Finished Selecting</div>');
                                echo('<div id="parseStop" ng-click="hideAvailable()">Cancel</div>');
                            echo('</div>');
                            echo('<div ng-repeat="obj in availableText track by obj.id" class="possibleTextBlock">');
                                echo('<div ng-class="obj.textClass" ng-click="handleText(obj.id)">{{obj.text}}</div>');
                                echo('<div class="parseOptionLine">');
                                    echo('<radiogroup value="$obj.id">');
                                        echo('<div class="parseOption"><input type="radio" id="Parsers0{{$index}}" name="parseAdd{{$index}}" ng-model="obj.parser" ng-value="Parsers[0]"><label for="Parsers0{{$index}}"><span></span>{{Parsers[0]}}</label></div>');
                                        echo('<div class="parseOption"><input type="radio" id="Parsers1{{$index}}" name="parseAdd{{$index}}" ng-model="obj.parser" ng-value="Parsers[1]"><label for="Parsers1{{$index}}"><span></span>{{Parsers[1]}}</label></div>');
                                        echo('<div class="parseOption"><input type="radio" id="Parsers2{{$index}}" name="parseAdd{{$index}}" ng-model="obj.parser" ng-value="Parsers[2]"><label for="Parsers2{{$index}}"><span></span>{{Parsers[2]}}</label></div>');
                                        echo('<div class="parseOption"><input type="radio" id="Parsers3{{$index}}" name="parseAdd{{$index}}" ng-model="obj.parser" ng-value="Parsers[3]"><label for="Parsers3{{$index}}"><span></span>{{Parsers[3]}}</label></div>');
                                        echo('<div class="parseOption"><input type="radio" id="Parsers4{{$index}}" name="parseAdd{{$index}}" ng-model="obj.parser" ng-value="Parsers[4]"><label for="Parsers4{{$index}}"><span></span>{{Parsers[4]}}</label></div>');
                                    echo('</radiogroup>');
                                echo('</div>');
                            echo('</div>');
                        echo('</div>');
